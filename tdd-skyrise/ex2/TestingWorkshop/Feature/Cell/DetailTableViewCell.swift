...
class DetailTableViewCell: UITableViewCell {
    ...
    func setupWithMeasurement(_ measurement: MeasurementList) {
        let firstMeasurement = measurement.data?.first
        let dateString = (firstMeasurement?.date.stringValue ?? "")
        
        nameLabel.text = measurement.message
            + "\n"
            + dateString
            + "\n"
            + ("\(converter.convertDegreesToWorldDirection(degrees: firstMeasurement?.value ?? 0.0))")
        
        typeImageView.image = UIImage(named: measurement.symbol.rawValue)
    }
